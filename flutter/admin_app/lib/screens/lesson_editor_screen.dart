...
class _LessonEditorScreenState extends State<LessonEditorScreen> {
  ...
  void initState() {
    ...
    final typeVal = widget.lesson?['type'];
    if (typeVal == 'video' || typeVal == 'audio' || typeVal == 'pdf' || typeVal == 'recording') {
      _type = typeVal;
    } else {
      _type = 'video';
    }
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        if (mounted) {
          ...
          messenger.showSnackBar(
            const SnackBar(content: Text('Lesson saved successfully'), backgroundColor: AppTheme.success)
          );
        }
      } else {
        if (mounted) {
