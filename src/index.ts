  ...
  try {
    ...
    try {
      ...
      if (dbError.message && dbError.message.includes("no such column")) {
        // Fallback for when avatar_url, updated_at, student_id etc. are missing during manual migrations
        user = (await env.DB.prepare("SELECT id, email, full_name, phone, district, state, country, birth_date, father_name, mother_name, grand_father_name, pincode, gender, bio, birth_place, role, created_at FROM Users WHERE id = ?")
          .bind(payload.sub)
          .first());
      } else {
        throw dbError;
...
async function chargeSelfStudyGroupClassIfNeeded(
  ...
  const rate = normalizeNonNegativeInt(session.live_class_credit_cost);
  const balance = await getCreditBalance(env, userId, "live_class");
...
async function handleEnrollBookBatch(
  ...
  try {
    ...
    const requiredCredits = normalizeNonNegativeInt(batch.live_class_credit_cost);
              ...
              if (!isAI && user?.role === "student" && sessionResult) {
                ...
                if (sessionResult.is_free !== 1 && !hasPaidEnrollment && !hasSubscriptionAccess && !hasLiveSessionAccess) {
                  ...
                  const creditAccessAvailable = creditPolicy &&
                    ...
                    Number(creditPolicy.live_class_credit_cost) > 0;
