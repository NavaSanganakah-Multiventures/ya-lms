...
async function callRealtimeAPI(env: Env, path: string, method: string, body: any, silent404: boolean = false) {
  try {
...
async function handleEndLiveSession(request: Request, env: Env, ctx?: ExecutionContext): Promise<Response> {
  try {
    ...
    if (recordingId) {
      ...
      await env.DB.prepare('UPDATE LiveSessions SET recording_id = ?, recording_status = "pending" WHERE id = ?').bind(recordingId, session.id).run();
    }

    return new Response(JSON.stringify({ success: true, message: "Meeting ended. Recording status saved." }), { status: 200 });
...
async function handleAdminProcessRecording(request: Request, env: Env, sessionId: string, ctx?: ExecutionContext): Promise<Response> {
  try {
    ...
    return new Response(JSON.stringify({ success: true, message: "Recording processing started/completed" }), { status: 200 });
  } catch (error) {
...
async function handleRecordingAction(request: Request, env: Env): Promise<Response> {
  try {
    ...
    if (action === "stop") {
      const activeData = await callRealtimeAPI(env, `/recordings/active-recording/${meetingId}`, 'GET', null, true);
      let recordingId = (activeData as any)?.data?.id || (activeData as any)?.result?.id;
...
async function initDbAndSeed(env: Env) {
  ...
  try {
    ...
    try {
      await env.DB.prepare(`ALTER TABLE LiveSessions ADD COLUMN recording_id TEXT;`).run();
    } catch (e) { /* Column already exists */ }
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      // Specific Course Sub-routes (Lessons, Live) - Check BEFORE general course routes
        ...
        return new Response(JSON.stringify({ token: geminiKey }), { status: 200, headers: { 'Content-Type': 'application/json' } });
      }
      else if (request.method === 'POST') {
