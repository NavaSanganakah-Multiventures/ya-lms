...
async function handleAdminAssignSubscription(
  ...
  try {
    ...
    const plan: any = await env.DB.prepare(
      ...
      .bind(planId)
      .first();
    const user: any = await env.DB.prepare("SELECT email, phone FROM Users WHERE id = ?")
      .bind(userId)
      .first();
...
`;
      ...
      const [userResult, enrollments, library, recentNotifications, examProgress] = await env.DB.batch([
        env.DB.prepare("SELECT email, full_name, phone, birth_date, father_name, mother_name, grand_father_name, district, state, country, pincode, bio, created_at FROM Users WHERE id = ?").bind(userId),
        env.DB.prepare(
          `
...
async function replaceDynamicVariables(
  ...
  const user = (await env.DB.prepare("SELECT id, full_name, email, role FROM Users WHERE email = ?")
    .bind(recipientEmail)
    .first()) as any;
  if (!user) return text;
