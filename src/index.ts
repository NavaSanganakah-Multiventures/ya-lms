...
async function initDbAndSeed(env: Env) {
  ...
  try {
    ...
    // Filter CREATE TABLE statements for parsing columns
    const tableMatches = [...SCHEMA_SQL.matchAll(/CREATE TABLE IF NOT EXISTS\s+([a-zA-Z0-9_]+)\s*\(([\s\S]*?)\);/gi)];
    
    // First, run all schema creation statements (SQLite ignores existing tables with IF NOT EXISTS)
    const createStatements = statements.map((s: string) => env.DB.prepare(s));
    for (let i = 0; i < createStatements.length; i += 50) {
       await env.DB.batch(createStatements.slice(i, i + 50));
    }

    // Next, auto-add missing columns for existing tables
    const expectedTables: { tableName: string, columns: {name: string, def: string}[] }[] = [];
    for (const match of tableMatches) {
      const tableName = match[1];
      const columnsDefStr = match[2];
      
      const lines = columnsDefStr.split(/,(?![^\(]*\))/);
      const expectedCols = [];
      
      for (let line of lines) {
         line = line.trim();
         if (!line || line.toUpperCase().startsWith('PRIMARY KEY') || line.toUpperCase().startsWith('FOREIGN KEY') || line.toUpperCase().startsWith('UNIQUE')) continue;
         
         const colMatch = line.match(/^([a-zA-Z0-9_]+)\s+(.*)$/i);
         if (colMatch) {
            expectedCols.push({ name: colMatch[1], def: colMatch[2].trim() });
         }
      }
      expectedTables.push({ tableName, columns: expectedCols });
    }

    const migrationStatements: any[] = [];
    
    // Check PRAGMA table_info for all expected tables
    for (let i = 0; i < expectedTables.length; i += 20) {
       const batch = expectedTables.slice(i, i + 20);
       const tableInfos = await env.DB.batch(
         batch.map((t) => env.DB.prepare(`PRAGMA table_info(${t.tableName})`))
       );
       
       tableInfos.forEach((res, idx) => {
         const expected = batch[idx];
         const existingCols = new Set(((res.results || []) as any[]).map((c: any) => c.name.toLowerCase()));
